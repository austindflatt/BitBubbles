...
//   const [search, setSearch] = useState('')
  ...
  <table class="currency-table">
	...
	{coins.map(row => {return (
      <tr>
        <td class="right">{row.market_data.market_cap_rank}</td>
        <td>
          <div class="name-cell">
            <button class="icon-button button-favorite" title="Add to favorites">
              <svg width="24" height="24">
                <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"></path>
              </svg>
            </button>
            <div class="currency-header"><a href={`coin/${row.id}`}
                key={row.symbol}><img src={row?.image.small} alt={row.symbol} title={`${row.symbol} Logo`} /><span>{row.name}</span></a></div>
          </div>
        </td>
        <td class="right">${abbreviateNumber(row.market_data.current_price.usd)}</td>
        <td class="right">${abbreviateNumber(row.market_data.market_cap.usd)}</td>
        <td class="right volume">0</td>
		<td class="right">${abbreviateNumber(row.market_data.circulating_supply)} {row.symbol.toUpperCase()}</td>
        <td class="center" style={{ 
										background:
											...
											: 'linear-gradient(rgb(255, 70, 71), rgb(219, 0, 0))', }}>
			{row.market_data.price_change_percentage_1h_in_currency.usd > 0 ? '+' : ''}
